...
| Food store capacity = piles + MAX_COLONIES × (pool + physical FoodStorage bound) | `constants.ts` (`MAX_COLONIES`, `FOOD_STORAGE_CHAMBERS_PER_COLONY_BOUND`, `FOOD_STORE_CAPACITY`) | capacity / colony count | guard added (typed-const tripwires on the grid + footprint constants and on `FOOD_STORE_CAPACITY` itself, which must stay ≤ 32766 for the Int16 `surfacePileAt` tile index; `food-store.test.ts` re-derives the bound; the loader rejects > MAX_COLONIES colonies) | #290 PR 2 |
