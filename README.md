...
| `npm run lint`      | ESLint                                                       |
| `npm run verify`    | Format check + lint + typecheck (src + scripts + tests + bench) + type-aware lint + sim-boundary/asset-path/e2e-geometry/layout-discipline/ant-cycle guards + unit tests (CI's verify job; e2e is a separate job) |
