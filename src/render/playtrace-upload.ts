...
export interface PlaytraceEnvelope {
   ...
   *  NOT bump PLAYTRACE_SCHEMA_VERSION on its own: the receiving Lambda stores
   *  the raw envelope, so an unknown extra key costs nothing and a reader that
   *  ignores it still sees a valid envelope at whatever version the round was
   *  submitted under (v3 since #294). Absent = the rule-based AI. */
