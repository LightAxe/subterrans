...
describe('difficultyDescription — derived from the sim triplets', () => {
  ...
  it('every number in a line is the tier constant it describes', () => {
    for (const tier of DIFFICULTY_TIERS) {
      ...
      expect(line).toContain(`hunts ${spiderHungerSeconds(tier)} s after eating`);
  ...
  it('pins the shipped wording (a retune must change this deliberately)', () => {
    expect(difficultyDescription('Easy')).toBe(
      'Enemy queen waits 25% longer between eggs. Enemy needs 10 fighters to arm, 18 to invade. Spider hunts 90 s after eating.',
    );
    expect(difficultyDescription('Normal')).toBe(
      'The reference tuning. Enemy needs 8 fighters to arm, 15 to invade. Spider hunts 60 s after eating.',
    );
    expect(difficultyDescription('Hard')).toBe(
      'Enemy queen waits 25% less between eggs. Enemy needs 6 fighters to arm, 12 to invade. Spider hunts 45 s after eating.',
    );
  });
  ...
  it('keeps each line short enough for two wrapped lines in the row', () => {
    ...
    for (const tier of DIFFICULTY_TIERS) {
      expect(difficultyDescription(tier).length).toBeLessThanOrEqual(126);
    }
  });
