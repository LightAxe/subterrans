...
// GameScene.notifyAutosaveFailed() is Phaser-coupled (it calls UIScene.showCaption),
// but its at-most-once-per-session guard + reset re-arm is a pure boolean fan-out —
// modeled here to pin the contract, the same way session-reset orchestration is
// modeled above (#234 PR2).
describe('#234 PR2 autosave-failure caption guard', () => {
  function makeGuard() {
    let notified = false; // GameScene.autosaveFailureNotified
    const captions: string[] = [];
    return {
      // Mirrors notifyAutosaveFailed(): guard, arm, emit the caption once.
      notifyAutosaveFailed(): void {
        if (notified) return;
        notified = true;
        captions.push('Autosave failed — storage full or blocked');
      },
      // Mirrors resetSessionState()'s `this.autosaveFailureNotified = false`.
      resetSession(): void {
        notified = false;
      },
      captions,
  ...
  it('fires the caption at most once per session, however many failures arrive', () => {
    const g = makeGuard();
    g.notifyAutosaveFailed();
    g.notifyAutosaveFailed();
    g.notifyAutosaveFailed();
    expect(g.captions).toHaveLength(1);
  });

  it('re-arms after resetSessionState so a fresh session whose storage is still full warns again', () => {
    const g = makeGuard();
    g.notifyAutosaveFailed();
    expect(g.captions).toHaveLength(1);
    g.resetSession();
    g.notifyAutosaveFailed();
    expect(g.captions).toHaveLength(2);
  });
});
