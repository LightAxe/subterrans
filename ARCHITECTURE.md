...
- **Tick dispatcher** (`tick.ts`) — `tick(world, commands)` applies the `SimCommand` array the platform game loop drained from `world.commandQueue` (the loop owns the `splice` and the replay logging — `game-loop.ts`; `tick` never drains the queue itself), then runs ~19 ordered steps each tick: reconcile colony stats → food consumption / starvation → death cleanup → queen egg production → lifecycle transitions → worker allocation → flow-field rebuild → task assignment → chamber/entrance completion → forage routing → pheromone deposit/decay → movement → forager/nurse/spider actions → combat → game-over check → RNG writeback.
